...
onMounted(() => {
  axios
    ...
    .then(
      axios.spread((reservationList, chambreList, clientList, factureList) => {
        let factures = factureList.data
        for (let el of reservationList.data) {
          const fact = factures.filter(elm => elm.booking == el.id)
          if (fact.length) {
            let toReduce = fact.map(elm => elm.amount).reduce((cum, curr) => cum + curr, 0)
...
const columns = [
...

  {
    ...
    label: "Dette Totale",
    align: "center",
    field: (row) => row.reservations.length ? row.reservations[0].dt : "",
    format: (val) => val?.toLocaleString("fr"),
    sortable: true,
  },
  {
    name: "dp",
    ...
    label: "Dette Payée",
    align: "center",
    field: (row) => row.reservations.length ? row.reservations[0].dp : "",
    format: (val) => val?.toLocaleString("fr"),
    sortable: true,
  },
  {
    ...
    label: "Payé Jour",
    align: "center",
    field: (row) => row.reservations.length ? row.reservations[0].pj : "",
    format: (val) => val?.toLocaleString("fr"),
    sortable: true,
  },
